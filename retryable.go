...
type RetryPolicy interface {
	DelayFor(int) (time.Duration, bool)
}
...
func (adapter *retryable) Delete(key string) error {
	return adapter.retry(func() error {
		return adapter.StoreAdapter.Delete(key)
	})
}
...
func (adapter *retryable) retry(action func() error) error {
	...
	for {
		...
		delay, keepRetrying := adapter.retryPolicy.DelayFor(attempt)
		if !keepRetrying {
			break
		}
